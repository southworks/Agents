...
test("implementation sessions use Copilot Auto and one validation tool", async () => {
  ...
  try {
    ...
    const runner = new CopilotAgentRunner(repo, "sample", { sdkVersion: "1.0.11", runtimeVersion: "1.0.83" }, path.join(repo, "agent.log"), [], [], async () => client);
    const active = await runner.open([{ name: "validate_sample", description: "Validate", parameters: { type: "object" }, handler: async () => ({}) }]);
    assert.equal(config?.model, "auto"); assert.equal(config?.capi, undefined); assert.equal(config?.tools?.length, 1); assert.equal(config?.tools?.[0]?.name, "validate_sample");
...
test("returns only the final assistant message while keeping the full transcript out of console output", async () => {
  ...
  try {
    ...
    const runner = new CopilotAgentRunner(repo, "sample", { sdkVersion: "1.0.11", runtimeVersion: "1.0.83" }, logFile, [], [], async () => client, () => {}, (value) => status.push(value));
