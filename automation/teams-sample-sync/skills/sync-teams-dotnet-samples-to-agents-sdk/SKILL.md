...
10. The trusted runner invokes a final read-only reviewer using both skills. The reviewer must account for the independent baseline and may revise it only with concrete evidence and a manifest-skill reference. An evidence-backed revision is part of the reviewer's effective final capability inventory and does not need a duplicate entry. Address validation errors and reviewer findings while preserving earlier changes and the cumulative report. Report-only correction calls are read-only. Stop after five implementation/review cycles, no progress, or any safety, agent-process, or infrastructure failure.
...
- An `updated` or `unchanged` result requires an existing manifest, a complete capability ledger, and one most-specific real manifest path for every `manifest-field-required` decision. Split behavior requiring multiple declarations into one capability per field. Code-only capabilities use `no-manifest-field`; they do not point at a generic parent manifest object.
- Capability IDs are lowercase correlation keys. Required manifest paths use concrete dotted properties and numeric array indexes; they never use wildcards or semantic selectors.
