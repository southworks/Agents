...
export function checkProject (sampleRoot: string, configured: Targets): { project?: string; errors: string[] } {
  ...
  let xml: unknown
  try { xml = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '' }).parse(readFileSync(project, 'utf8')) } catch (error) { return { project, errors: [`Invalid project XML: ${error instanceof Error ? error.message : String(error)}`] } }
...
export async function checkManifest (sampleRoot: string, manifestTarget: ManifestTarget, loadSchema = fetchSchema): Promise<string[]> {
  ...
  let url: URL
  try { url = new URL(schemaUrl) } catch { errors.push('Manifest $schema is not a valid URL'); return errors }
