...
import { blockingRecord, prBody, workflowSummary } from "./report.js";
...
export async function migrateCandidate(repo: string, values: Record<string, string>, dependencies: { sdkFactory?: SdkFactory; validationRuntime?: ValidationRuntime } = {}): Promise<number> {
  ...
  let acceptedAgent: AgentResult | undefined; let acceptedReview: ReviewResult | undefined; let preparedCandidateDigest: string | undefined;
  ...
  try {
    ...
    preparedCandidateDigest = digestDirectory(sampleRoot, owner.outputDigestExcludes);
    ...
    if (result.stage === "blocked" || result.agent.status === "needs-policy" || result.agent.status === "unsupported") {
      syncResult.status = result.agent.status === "needs-policy" || result.agent.status === "unsupported" ? result.agent.status : "failed";
      syncResult.error = result.review?.result.blockerReason ?? result.agent.summary; syncResult.failureClass = "blocked"; syncResult.failureStage = result.stage;
      if (digestDirectory(sampleRoot, owner.outputDigestExcludes) !== preparedCandidateDigest) throw new SyncError("A reportable outcome must not leave partial candidate edits");
      const reportRelative = `automation/teams-sample-sync/reports/${sample}.md`;
      syncResult.publicationKind = "report"; syncResult.publishable = true; syncResult.reportPath = reportRelative; syncResult.destinationChanges = [reportRelative];
      writeFileSync(path.join(repo, reportRelative), blockingRecord(syncResult), "utf8");
      git(repo, ["add", "-N", "--", reportRelative]);
      const patch = git(repo, ["diff", "--binary", baseSha, "--", reportRelative], true) as Buffer;
      if (patch.length === 0) throw new SyncError("Reportable outcome produced no report patch");
      ...
      syncResult.status = "updated"; syncResult.publishable = true; syncResult.publicationKind = "update"; syncResult.destinationChanges = changedPaths(repo, baseSha).filter((item) => item === path.relative(repo, lock).replaceAll("\\", "/") || item.startsWith(`${sampleRelative}/`)); syncResult.state = state;
    }
...
function verifyPatch(repo: string, values: Record<string, string>): void {
  ...
  if (result.sample !== sample || !result.publishable) throw new SyncError("Result is not marked for publication");
  if (result.publicationKind === "report") { verifyReportPatch(repo, resultFile, result, sample); return; }
  if (result.publicationKind !== "update" || result.status !== "updated" || !result.state || !result.outputDigest || !result.evidenceDigest || !result.validation?.passed || !result.agent || !result.review || result.review.result.verdict !== "approved") throw new SyncError("Only a complete v3 updated result is publishable");
...
function verifyReportPatch(repo: string, resultFile: string, result: SyncResult, sample: string): void {
  if ((result.status !== "needs-policy" && result.status !== "unsupported") || !result.agent || !result.reportPath || result.destinationChanges?.length !== 1 || result.destinationChanges[0] !== result.reportPath) throw new SyncError("Only a structured needs-policy or unsupported result can publish a report");
  if (result.reportPath !== `automation/teams-sample-sync/reports/${sample}.md`) throw new SyncError("Report path does not match the selected sample");
  const contextFile = path.join(path.dirname(resultFile), "source-context.json"); if (!existsSync(contextFile)) throw new SyncError("Trusted source context artifact is missing");
  const context = readJson<SyncContext>(contextFile); if (hash(stable(context)) !== result.sourceContextDigest || context.sample !== sample || context.upstream.currentCommit !== result.upstreamCommit || context.upstream.currentTree !== result.sourceTree) throw new SyncError("Source context does not match the reportable result");
  const agent = parseAgentResult(result.agent, sample); if (agent.status !== result.status) throw new SyncError("Agent status does not match reportable result");
  if (stable(result.migrationPolicies) !== stable(context.policies)) throw new SyncError("Reported policies differ from source context");
  const configured = targets(repo); if (stable(result.copilot) !== stable(configured.copilot) || result.componentDigests.copilot !== hash(stable(result.copilot))) throw new SyncError("Copilot configuration differs from reportable result");
  const head = git(repo, ["rev-parse", "HEAD"]) as string; if (head !== result.baseSha) throw new SyncError("Publish checkout differs from reportable base SHA");
  const changed = changedPaths(repo, head); if (stable(changed) !== stable([result.reportPath])) throw new SyncError("Report patch changes prohibited paths");
  const report = path.join(repo, result.reportPath); if (!existsSync(report) || readFileSync(report, "utf8") !== blockingRecord(result)) throw new SyncError("Applied report does not match the structured result");
  const patchFile = path.join(path.dirname(resultFile), "change.patch"); if (!existsSync(patchFile)) throw new SyncError("Report patch is missing"); git(repo, ["apply", "--check", "--reverse", "--binary", patchFile]);
}
