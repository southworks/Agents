...
export function text (value: unknown, name: string): string {
  if (typeof value !== 'string' || value.trim() === '') {
    throw new SyncError(`${name} is required`)
  }
  return value
}
...
export function containedPath (root: string, relative: string): string {
  if (!relativePath(relative)) {
    throw new SyncError(`Unsafe repository path: ${relative}`)
  }
  const candidate = path.resolve(root, relative)
  const back = path.relative(root, candidate)
  if (back.startsWith(`..${path.sep}`) || path.isAbsolute(back)) {
    throw new SyncError(`Path is outside repository: ${relative}`)
  }
  return candidate
}
...
function manifest (value: unknown, name: string): ManifestTarget {
  ...
  if (!relativePath(result.packageDirectory)) {
    throw new SyncError(`${name}.packageDirectory is unsafe`)
  }
  return result
}
...
export function targets (repo: string): Targets {
  ...
  if (value.version !== 1) {
    throw new SyncError('targets.yml must use version 1')
  }
  ...
  for (const [name, raw] of Object.entries(rawSamples)) {
    ...
    if (!relativePath(source) || !relativePath(destination)) {
      throw new SyncError(`Sample ${name} has unsafe path`)
    }
    if (destinations.has(destination)) {
      throw new SyncError(`Duplicate destination: ${destination}`)
    }
  ...
  const result: Targets = {
    ...
    copilot: {
      sdkVersion: text(copilot.sdkVersion, 'copilot.sdkVersion'),
      runtimeVersion: text(copilot.runtimeVersion, 'copilot.runtimeVersion'),
    },
    packagePolicy: {
      targetFramework: text(packagePolicy.targetFramework, 'packagePolicy.targetFramework'),
      minimumAgentsSdkVersion: text(packagePolicy.minimumAgentsSdkVersion, 'packagePolicy.minimumAgentsSdkVersion'),
    },
    validatorVersion: text(value.validatorVersion, 'validatorVersion'),
    samples,
  }
  if (!/^\d+\.\d+\.\d+$/.test(result.copilot.sdkVersion) || !/^\d+\.\d+\.\d+$/.test(result.copilot.runtimeVersion)) {
    throw new SyncError('Copilot SDK and runtime versions must be exact versions')
  }
  if (!/^\d+\.\d+$/.test(result.packagePolicy.minimumAgentsSdkVersion)) {
  ...
  for (const [name, candidate] of Object.entries({
    ...
    if (!relativePath(candidate)) {
      throw new SyncError(`targets.yml has unsafe ${name}`)
    }
  }
  return result
}
export function protection (repo: string): Protection {
  ...
  if (value.version !== 1) {
    throw new SyncError('ownership.yml must use version 1')
  }
  return {
    version: 1,
