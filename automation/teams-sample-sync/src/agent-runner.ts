...
export function permissionFor (repo: string, sampleRoot: string, writeEnabled: boolean, customTools: string[], request: PermissionRequest): PermissionRequestResult {
  if (request.kind === 'read') {
    try { const root = realpathSync(repo); const candidate = realpathSync(path.resolve(repo, request.path)); const relative = path.relative(root, candidate); const segments = relative.split(path.sep); return { kind: candidate === root || (!relative.startsWith('..') && !path.isAbsolute(relative) && !segments.includes('.git') && !segments.includes('.codex')) ? 'approve-once' : 'reject' } } catch { return { kind: 'reject' } }
  }
  if (request.kind === 'url') {
    try { const url = new URL(request.url); const official = ['learn.microsoft.com', 'developer.microsoft.com', 'docs.github.com', 'github.com', 'raw.githubusercontent.com', 'www.nuget.org', 'api.nuget.org']; return { kind: url.protocol === 'https:' && official.includes(url.hostname) && !url.username && !url.password ? 'approve-once' : 'reject' } } catch { return { kind: 'reject' } }
  }
