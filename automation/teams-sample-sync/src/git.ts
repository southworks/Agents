...
function gitResult (root: string, args: string[], binary: boolean, allowFailure: boolean): string | Buffer | undefined {
  ...
  if (result.status !== 0) {
    ...
    throw new SyncError(
      `Git failed: ${args.join(' ')}: ${(stderr || stdout || result.error?.message || 'unknown error').trim()}`
    )
  }
...
export function matches (relative: string, patterns: string[]): boolean {
  ...
  return patterns.some(
    (pattern) => minimatch(normalized, pattern, { dot: true }) || minimatch(normalized, `**/${pattern}`, { dot: true })
  )
}
  ...
  const visit = (directory: string): void => {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) {
        throw new SyncError(`Symlink is not allowed: ${file}`)
      }
...
export function changedPaths (repo: string, base: string): string[] {
  const tracked = (git(repo, ['diff', '--name-only', '-z', base], true) as Buffer)
    .toString('utf8')
    .split('\0')
    .filter(Boolean)
  const untracked = (git(repo, ['ls-files', '--others', '--exclude-standard', '-z'], true) as Buffer)
    .toString('utf8')
    .split('\0')
    .filter((item) => item !== '' && !item.startsWith('.sync/'))
...
export function upstreamChanges (repo: string, previous: string, current: string, sourceRoot: string): UpstreamChange[] {
  const raw = git(
    repo,
    ['diff', '--name-status', '-z', '--find-renames', previous, current, '--', sourceRoot],
    true
  ) as Buffer
  ...
  const strip = (item: string): string =>
    item.replaceAll('\\', '/').startsWith(prefix) ? item.replaceAll('\\', '/').slice(prefix.length) : item
  for (let index = 0; index < fields.length - 1;) {
    ...
    result.push({
      ...
      binary:
        binaryBlob(repo, status === 'added' ? current : previous, oldRepositoryPath) ||
...
export function materializeTree (repo: string, ref: string, sourceRoot: string, destination: string): void {
  ...
  for (const entry of raw.toString('utf8').split('\0').filter(Boolean)) {
    ...
    if (tab < 0) {
      throw new SyncError('Unexpected git ls-tree output')
    }
    const metadata = entry.slice(0, tab).split(' ')
    const repositoryPath = entry.slice(tab + 1).replaceAll('\\', '/')
    const mode = metadata[0]
    if (mode === '120000') {
      throw new SyncError(`Symlink is not allowed in upstream tree: ${repositoryPath}`)
    }
    ...
    if (!relativePath(relative)) {
      throw new SyncError(`Unsafe upstream tree path: ${repositoryPath}`)
    }
    ...
    for (let parent = path.dirname(output); parent !== destination; parent = path.dirname(parent)) {
      if (existsSync(parent) && lstatSync(parent).isSymbolicLink()) {
        throw new SyncError(`Symlink is not allowed: ${parent}`)
      }
    }
