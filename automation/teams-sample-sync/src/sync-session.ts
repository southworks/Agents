...
export async function runMigrationSession (options: MigrationSessionOptions): Promise<MigrationSessionResult> {
  ...
  const timer = setTimeout(() => { expired = true; options.session.abort().catch(() => {}); rejectDeadline(new SyncError('Per-sample migration deadline exceeded')) }, options.deadlineMs ?? 30 * 60_000)
  ...
  const completedSelfAudit = async (value: string): Promise<{ selfAudit: string; outcome: MigrationOutcome }> => {
    try { return { selfAudit: value, outcome: outcomeFromSelfAudit(value) } } catch {}
    ...
    try { return { selfAudit, outcome: outcomeFromSelfAudit(selfAudit) } } catch {}
